...
model** (Qwen2.5-7B-Instruct) answers using **only what it retrieves from your
documents**, and cites where each answer came from. When an answer is wrong, you correct
it -- those corrections become training data to fine-tune the model to do better next time.
...
Start with an **uncustomized** Qwen2.5-7B-Instruct and good retrieval. That alone answers
a lot of questions well. Where it falls short -- ignores the context, rambles, fails to
say "I don't know," or formats poorly -- you flag the answer and write the version you
wanted. Those `(question, retrieved-context, good-answer)` examples accumulate into a
training set, and you periodically fine-tune the model on them (on Colab, with your own
...
- **Embed gate** -- serializes the **vector model** (CPU-pinned). Two callers share it:
  **ingestion** (embedding a dropped file's chunks) and **retrieval** (embedding the
  ...
  the whole file.
- **Gen gate** -- serializes the **language model** (Qwen2.5-7B, on the GPU when present).
  One caller: answering. Two questions never generate at once.
...
and it leaves **all** VRAM to the generator (weights + KV cache). The language model takes the GPU
...
2. When you have enough, run the Colab notebook: an Unsloth LoRA fine-tune on those
   examples -> export Q4_K_M GGUF -> upload to your Hugging Face account.
3. Restart `serve.py`; its staleness check pulls the new weights and serves them.

> **Current status of the loop:** serving runs **Qwen2.5-7B-Instruct**, but the notebook
> (`training/finetune_phi_rag.ipynb`) still fine-tunes **Phi-4-mini** -- it predates the
> generator swap. Porting the fine-tune target to Qwen (so step 3 serves what step 2
> trained) is pending, as are correction-capture (P9) and the auto-pull staleness check
> (P11). Grounded Q&A with the base Qwen model works today; the self-improvement loop does
> not yet close end-to-end.
...
- **Qwen2.5-7B-Instruct for generation** *(Apache-2.0)*. Chosen from a Colab
  model x prompt eval as the best grounded-answer quality at a size that still runs
  locally via `llama.cpp` (Q4_K_M on the GPU when it fits, CPU otherwise). "Uncustomized"
  to start -- the base instruct GGUF. It needs **very little background knowledge baked
  in**: it is instructed to ignore what it "knows" and answer **only from the retrieved
  document context**, so comprehension and grounding matter far more than world knowledge
  here. (It replaced Phi-4-mini, which the fine-tune notebook still targets -- see the
  improvement-loop note above.)
...
- **A self-contained portable runtime.** Bundled `uv` + `llama.cpp`, `run.bat`/`run.sh`,
  ...
  run side by side: one for the generator (chat), one for the embedder. No global install step.
...
the Qwen2.5-7B and embedder GGUFs from Hugging Face on first run (smallest first, so you
can start as soon as the embedder is ready), then serves the UI. Open
<http://localhost:51548>.

| Port | Process |
|------|---------|
| 51548 | `serve.py` -- web UI + API + SQLite/sqlite-vec |
| 52851 | `llama-server` -- Qwen2.5-7B-Instruct (generation) |
...
- **`./logs/` is the append-only audit trail.** One JSON object per
  ...
  sent to the generator, the reply, token counts / latency, and any error. Grep the **chat session
